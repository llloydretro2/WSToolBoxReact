					...
					<div className="flex gap-4">
						...
						<div className="flex-1 flex flex-col gap-2 min-w-0">
							<div className="flex flex-wrap gap-1.5">
								...
								{card.card_type && (
									<span className="text-[10px] font-bold px-2 py-0.5 rounded-full border border-[var(--border)] text-[var(--text-secondary)]">{t(`enCardList.cardTypes.${card.card_type}`) || card.card_type}</span>
								)}
							</div>
								...
								<div className="flex gap-1.5">
									{CARD_TYPE_OPTIONS.map((type) => (
										<button
											...
											{t(`enCardList.cardTypes.${type}`) || type}
										</button>
									))}
								</div>
							</div>
				...
				<CardDetailModal
					...
					onRelatedCardClick={async (rc) => {
						try {
							...
							const res  = await apiRequest(`/api/cards/en?${params}`);
							const data = await res.json();
