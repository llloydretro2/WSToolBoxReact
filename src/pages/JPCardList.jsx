...
const COLOR_OPTIONS = [
	...
	{ value: "purple", label: "Purple", dot: "bg-purple-500", pill: "bg-purple-100 text-purple-800" },
];
...
const CARD_TYPE_OPTIONS = ["キャラ", "イベント", "クライマックス"];
...
function NeoCombobox({ value, onChange, jpMap, cnMap }) {
	const { t } = useLocale();
	const [query, setQuery] = useState("");

	const items = useMemo(() =>
		Object.keys(jpMap).sort().map((key) => ({
			key,
			label: cnMap?.[key] ? `${key}（${cnMap[key]}）` : key,
		})),
		[jpMap, cnMap]
	);
	...
	const displayLabel = value
		? (cnMap?.[value] ? `${value}（${cnMap[value]}）` : value)
		: "";

	return (
		<Combobox value={value} onChange={(v) => onChange(v ?? "")} onClose={() => setQuery("")} immediate>
			<div className="relative">
				<Combobox.Input
					autoComplete="off"
					readOnly={!!value}
...
NeoCombobox.propTypes = {
	value: PropTypes.string.isRequired,
	onChange: PropTypes.func.isRequired,
	jpMap: PropTypes.object.isRequired,
	cnMap: PropTypes.object,
};

// ── Card image (climax 90° rotation) ─────────────────────────────────────────

const CLIMAX_IMG_STYLE = {
	position: "absolute", width: "140%", height: "71.43%",
	top: "14.285%", left: "-20%", transform: "rotate(90deg)", borderRadius: "0",
};
...
function RangeSelect({ value, options, onChange, formatOption }) {
	...
	return (
		<div className="flex items-center gap-2 w-full">
			<ValueListbox value={minVal} options={options.filter((o) => o <= maxVal)} onChange={(v) => onChange([v, maxVal])} formatOption={formatOption} />
			<span className="text-xs text-[var(--text-muted)] shrink-0">—</span>
			<ValueListbox value={maxVal} options={options.filter((o) => o >= minVal)} onChange={(v) => onChange([minVal, v])} formatOption={formatOption} />
		</div>
	);
}
...
function CardDetailModal({ card, onClose, onRelatedCardClick }) {
	...
	const sideLabel = SIDE_OPTIONS.find((o) => o.value === card.side)?.label ?? "";
			...
			<div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl overflow-hidden mb-8">
				...
				<div className="px-5 py-4 bg-[var(--text-muted)] flex items-center justify-between">
					<div className="min-w-0 flex-1">
						...
						{card.zh_name && <p className="text-xs text-white/80 truncate">{card.zh_name}</p>}
				...
				<div className="p-5 flex flex-col gap-4">
						...
						<div className="flex-1 flex flex-col gap-2 min-w-0">
							...
							{card.feature && (
								<div>
									...
									{card.zh_trait && <p className="text-xs text-[var(--text-muted)]">{card.zh_trait}</p>}
								</div>
							)}
						</div>
					</div>
					...
					{/* Expansion / series */}
					{(card.series_number || card.series || card.product_name) && (
						<div>
							...
							{(card.series_number || card.series) && (
								<p className="text-xs text-[var(--text-secondary)]">
									{[card.series_number, card.series].filter(Boolean).join(" · ")}
								</p>
							)}
							{card.product_name && <p className="text-sm text-[var(--text)]">{card.product_name}</p>}
						</div>
					)}
					...
					{card.effect && (
						<div className="border border-[var(--border)] rounded-xl p-3">
							<p className="text-[9px] font-black tracking-widest uppercase text-[var(--text-muted)] mb-2">{t("enCardList.labelEffect")}</p>
							<p className="text-xs text-[var(--text)] whitespace-pre-line leading-relaxed">{card.effect}</p>
							{card.zh_effect && (
								<p className="text-xs text-[var(--text-secondary)] whitespace-pre-line leading-relaxed mt-2 pt-2 border-t border-[var(--border)]">
									{card.zh_effect}
								</p>
							)}
						</div>
					...
					{card.flavor && (
						<div className="border border-[var(--border)] rounded-xl p-3 bg-[var(--card-background)]">
							...
							{card.zh_flavor && (
								<p className="text-xs italic text-[var(--text-muted)] mt-1">{card.zh_flavor}</p>
							)}
						</div>
					)}
								...
								{card.related_cards.map((rc) => (
									<button key={rc.cardno}
										...
										<div className="min-w-0">
											...
											{rc.zh_name && <p className="text-[10px] text-[var(--text-muted)] truncate">{rc.zh_name}</p>}
...
const JPCardList = () => {
	const { t } = useLocale();
	const { productList, jpNeostandardMap, translationMap, optionsLoading } = useOptions();

	// ── Derived filter option arrays ──────────────────────────────────────────
	const validLevels = useMemo(() =>
		(productList.level ?? []).filter((v) => v !== "-").map(Number).sort((a, b) => a - b),
		[productList.level]
	);
	const validCosts = useMemo(() =>
		(productList.cost ?? []).filter((v) => v !== "-").map(Number).sort((a, b) => a - b),
		[productList.cost]
	);
	const validPowers = useMemo(() =>
		(productList.power ?? []).filter((v) => v !== "-").map(Number).sort((a, b) => a - b),
		[productList.power]
	);
	...
	// Initialise ranges once options load
	useEffect(() => {
	...
	const fetchCards = useCallback(async (searchParams, pg) => {
		...
		try {
			...
			const res  = await apiRequest(`/api/cards/jp?${searchParams.toString()}`);
			const data = await res.json();
			setCards(data.data  ?? []);
			setTotal(data.total ?? 0);
		} catch (err) {
			console.error("JP card fetch failed:", err);
			setCards([]);
			setFetchError(true);
		} finally {
			setLoading(false);
	...
	const buildParams = useCallback((d, neo, soul, lr, cr, pr) => {
		...
		if (neo) {
			const codes = jpNeostandardMap[neo];
			if (codes?.length) params.set("series_number", codes.join(","));
		}
		// JP backend: soul=1 (numeric), not soul_min
		if (soul) params.set("soul", "1");
		...
		return params;
	}, [jpNeostandardMap, maxLevel, maxCost, validPowers]);
	...
	const cardGroups = useMemo(() => {
		...
		cards.forEach((c) => {
			...
			if (!map.has(base)) { map.set(base, { key: base, variants: [] }); order.push(base); }
	...
	const activeCount = (
		...
		+ (draftNeostandard ? 1 : 0)
		+ (draftSoul ? 1 : 0)
		+ (levelRange[0] > 0 || levelRange[1] < maxLevel ? 1 : 0)
		+ (costRange[0]  > 0 || costRange[1]  < maxCost  ? 1 : 0)
		+ (powerRange && validPowers.length > 0 && (
			powerRange[0] > validPowers[0] ||
			powerRange[1] < validPowers[validPowers.length - 1]
		) ? 1 : 0)
	);
	...
	return (
		<div className="max-w-5xl mx-auto px-4 sm:px-6 pb-8 sm:py-10">
			...
			<div className="mb-6">
				...
				<p className="text-sm text-[var(--text-secondary)]">{t("pages.cardList.subtitle")}</p>
			</div>
					...
					<div className="px-5 pb-5 border-t border-[var(--border)] pt-4 flex flex-col gap-3">
						...
						<div className="grid grid-cols-2 gap-3">
							<div>
								...
								<NeoCombobox
									...
									onChange={setDraftNeostandard}
									jpMap={jpNeostandardMap}
									cnMap={translationMap?.neostandard}
								/>
							</div>
							<div>
								<label className={`text-[10px] font-black tracking-widest uppercase mb-1 flex items-center gap-1.5 transition-colors ${
						...
						<div className="flex items-center justify-center gap-3 pt-2 border-t border-[var(--border)]">
							<button
								...
								className="px-8 py-2 rounded-xl bg-[var(--text-muted)] text-white text-sm font-bold hover:bg-[var(--text-secondary)] active:scale-95 transition-all shadow-sm disabled:opacity-40 disabled:cursor-not-allowed">
						...
						return (
							<div key={key} onClick={() => setSelectedCard(card)}
								...
								<div className="px-2 py-1.5 bg-white flex flex-col gap-0.5">
									...
									{card.zh_name && (
										<p className="text-[9px] text-[var(--text-muted)] leading-tight line-clamp-1">{card.zh_name}</p>
									)}
									{variants.length > 1 ? (
										<div className="flex flex-wrap gap-0.5 mt-0.5">
											{variants.map((v, i) => (
												<button key={v.cardno ?? i}
