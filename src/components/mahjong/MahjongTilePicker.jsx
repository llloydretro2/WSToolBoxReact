    ...
    <div>
      ...
      <div className="flex flex-col gap-3">
        {GROUPS.map(({ key, short, tiles }) => (
          <div key={key} className="flex items-center gap-3">
            <span className="w-5 text-center text-[11px] font-black text-[var(--text)] shrink-0 select-none">
              {short}
            </span>
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        <button
          ...
          className="flex items-center gap-1 text-[11px] font-bold px-3 py-1 rounded-full bg-[var(--text-muted)] text-white hover:bg-[var(--text-secondary)] transition-colors shrink-0"
        >
        ...
        {meldOpen && (
          <div className="mt-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`text-[11px] font-bold px-3 py-1 rounded-full transition-colors ${
                    canAddMeld ? 'bg-[var(--text-muted)] text-white hover:bg-[var(--text-secondary)]' : 'text-[var(--text)] cursor-not-allowed'
                  }`}
                >
                ...
                {meldBuilder.length > 0 && (
                  <button
                    ...
                    className="text-[11px] text-[var(--text)] hover:text-[var(--text)] transition-colors"
                  >
                    {locale === 'zh' ? '清空' : 'Clear'}
                  </button>
                )}
            ...
            {meldBuilder.length >= 3 && (
              <p className={`text-[11px] mt-1.5 mb-2 ${meldValidation.valid ? 'text-[var(--text)]' : 'text-[var(--text)]'}`}>
            ...
            <div className="flex flex-col gap-2">
              {GROUPS.map(({ key, short, tiles }) => (
                <div key={key} className="flex items-center gap-2">
                  <span className="w-4 text-center text-[10px] font-black text-[var(--text)] shrink-0">
                    {short}
                  </span>
