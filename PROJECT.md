...
- **WS 伤害计算器** `/ws/damage`：帮助玩家量化攻击伤害预期的计算工具。纯前端，无需后端。
  ...
  **B1 peek-return 概率分析**：
  ...
  ### 前端页面开发状态（2026-06-01，session 38–39）
  ...
  **页面文件**：`src/pages/DamageCalculator.jsx`
  ...
  - **牌库状态**：总张数 + 高潮数（上限 60）
  - **弃牌堆状态**：总张数 + 高潮数
  - **血量状态**：等级（0–3）+ 血量（0–6）（i18n：zh 等级/血量，en Level/Clock）

  #### 伤害序列区
  8 种预设模板（badge 颜色通过 inline `style` 实现，避免 Tailwind purge）：

  | 模板 ID | 标签 | 参数 | 引擎映射 |
  |---------|------|------|---------|
  | `direct` | 直接伤害 | `n` | `Damage(n)` |
  | `cancel` | 取消追加 | `n, m, times` | `Damage(n, onCancel: [Damage(m)]×times)` |
  | `bottom_flip` | 翻底X潮次伤害 | `n, perClimax, dmg, times` | MoveOp(bottom n) + ConditionalOp×n |
  ...
  | `top_remove_cx` | 看X顶送潮入墓 | `n` | MoveOp(top n, CX→rest, non-CX original→source) |
  | `cancel_return` | 取消后X洗回卡组 | `n, y` | `Damage(n, onCancel: [FX(y, non_climax)])` |
  | `return_cx` | 洗X非潮回卡组 | `n` | `FX(n, non_climax)`（独立步骤，直接回库洗牌） |

  步骤数值输入使用 `StepInput` 步进器（`−`/`+` 按钮 + 中间文本框允许清空，blur 时 clamp）。
  ...
  - 辅助统计：期望进血量张数 / 平均刷新次数 / 平均升级次数
  - **伤害分布区**（`DistributionSection`）：
    - SVG 柱状图：从 0 到 max 每个值一根柱，overflow-x-auto 横滚
    - 概率列表：3/4 列网格，每行 `k 张 | X.X%`
    - 7/14/21 升级阈值高亮（深色背景）

  #### i18n
  页面已接入 `useLocale()`，`src/locales/zh.json` + `en.json` 新增顶级 `damage` section：`levelLabel`/`clockLabel`/`expectedClock`/`avgRefresh`/`avgLevelUp`/`killRate`
...
- **WS 卡片 DIY 制作页面** `/ws/card-maker`（2026-06-01 计划完成）
  ...
  **已实现（Phase 2）**
  ...
  |------|------|
  | 事件卡渲染（框架已有，坐标差异处理） | ✅ |
  | 高潮卡渲染（626×448，angle=270 坐标变换） | ✅ 基本可用 |
  | 卡图编辑器（拖拽平移 + 缩放，触摸支持） | ✅ |
  | 高潮卡触发图标位置（左上角，双图标垂直叠放） | ✅ 来自 style 文件 |
  | 高潮卡卡名 / 卡号坐标（来自 style 文件） | ✅ |
  | 高潮卡风味文字独立渲染（大字，右侧美术区） | ✅ |
  | 高潮卡效果文字（左下白框，w=204） | ✅ 基本可用 |

  **高潮卡坐标变换公式（已验证）**

  style 文件坐标系（`angle: 270`）→ 626×448 landscape 画布：
  ```
  landscape_x     = style_top
  landscape_y_bot = 447 - style_left
  landscape_y_top = 447 - (style_left + style_h)
  ```
  对图像元素完全准确（trigger/cardname/serial 均验证通过）。

  **已知问题：高潮卡文字 Y 坐标偏差**

  效果文字和风味文字的 Y 坐标无法完全从 style 文件公式推导：
  - 效果文字：style `left=17` → 公式给 `bottomY=430`，实测应为 `381`，差 49px
  - 风味文字：style `left=65` → 公式给 `bottomY=382`，实测应为 `320`，差 62px
  - 偏差不一致，疑似与 Bar 位置锚定或 MSE 内部底部对齐逻辑有关
  - 当前用实测值暂代，后续如有条件运行 MSE 可精确校正

  **已知待优化（暂缓）**
  - 效果文本内联符号（`{ACT}` `{AUTO}` 等→小图标）
  - 高潮卡文字 Y 坐标精确校正（见上方已知问题）
  - SP/foil 闪卡效果（暂不做）
